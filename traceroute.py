...
class TracerouteClone:
    ...
    def trace(self, hostname, num_samples = 3, max_hops = 30, timeout = 2):
        ...
        * https://scapy.readthedocs.io/en/latest/api/scapy.layers.inet.html#scapy.layers.inet.ICMP
        ...
        #loop over each TTL value from 1 up to the max number of hops allowed
        for ttl in range(1, max_hops + 1):  #send the specified number of sample packets at each TTL level
            for _ in range(num_samples):
                pkt = IP(dst = target, ttl=ttl) / ICMP()    #ICMP packet that will stop at this TTL
                start_time = time.time()                         
                reply = sr1(pkt, verbose=0, timeout=timeout)  
                end_time = time.time()
                if reply is None:   #no reply received, likely a timeout, log this as no response
                    self.update_traceroute_log(ttl, ip_address="*", rtt="*")
                else:             
                    rtt = (end_time - start_time) * 1000  #calculate the round trip time in milliseconds
                    ip_address = reply.src  #source IP address from the received packet
                    self.update_traceroute_log(ttl, ip_address = ip_address, rtt = rtt)
                    if reply.type == 0:  
                        return self.log
